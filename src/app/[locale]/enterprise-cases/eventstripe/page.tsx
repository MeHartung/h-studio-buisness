...
export async function generateMetadata({
  ...
  const metadata = messages.enterpriseCases?.eventstripe?.meta || {
    title: `EventStripe - Enterprise Case Study | H-Studio`,
    description: "Высоконагруженная SaaS-платформа для продажи билетов"
  };
  
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.h-studio-tech.ru';
  const currentUrl = `${baseUrl}/${locale}/enterprise-cases/eventstripe`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ru: `${baseUrl}/ru/enterprise-cases/eventstripe`,
        en: `${baseUrl}/en/enterprise-cases/eventstripe`,
        de: `${baseUrl}/de/enterprise-cases/eventstripe`,
      },
    },
    openGraph: {
      title: "EventStripe - Enterprise Case Study",
      description: "Высоконагруженная SaaS-платформа для продажи билетов",
      url: currentUrl,
      siteName: "H-Studio",
      images: [
        {
          url: `${baseUrl}/images/blog/EventStripe/cover/cover.webp`,
          width: 1200,
          height: 630,
          alt: "EventStripe Case Study",
        },
      ],
...
export default async function EventStripeCase({
  params
}: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  
  const t = await getTranslations('enterpriseCases.eventstripe');
  const tc = await getTranslations('enterpriseCases.eventstripe.content');
  ...
  const breadcrumbItems = [
    ...
    { name: t('title'), url: `${baseUrl}/${locale}/enterprise-cases/eventstripe` },
  ];
              ...
              <div className="w-full max-w-5xl">
                <div className="prose prose-invert max-w-none">
                  <div className="relative">
                    <Image
                      src="/images/blog/EventStripe/cover/cover.webp"
                      alt={t('coverImageAlt')}
                      width={300}
                      height={225}
                  ...
                  <div className="mb-8 space-y-3">
                    ...
                    {tc.has('techStack.frontend') && (
                      <p className="text-base text-text/70"><strong className="text-text">Frontend:</strong> {tc('techStack.frontend')}</p>
                    )}
                    ...
                    {tc.has('techStack.monitoring') && (
                      <p className="text-base text-text/70"><strong className="text-text">Monitoring:</strong> {tc('techStack.monitoring')}</p>
                    )}
